...
class TrackedObject:
    def __init__(self, object_id, class_id, bbox, confidence, alpha_dist=0.2, alpha_speed=0.1, alpha_motion=0.2):
        self.object_id = object_id
        self.class_id = class_id
        self.bbox = bbox  # (x1, y1, x2, y2)
        self.confidence = confidence
        self.prev_centroid = self.calculate_centroid()
    ...
    """
    Direction decider based on relative speed and box growth:
    """
    def direction_decider(self):
        gr_oncoming = 1.03 
        gr_stationary = 1.005 
        v_stationary_max = 5.0 

        if abs(self.real_speed) < v_stationary_max and self.width_box_growth < gr_oncoming:
            return "STATIONARY"

        if self.width_box_growth > gr_oncoming:
            return "ONCOMING"

        if self.real_speed > v_stationary_max and self.width_box_growth < gr_oncoming:
            return "FOLLOWING"

        return "STABLE"
    
    """
    Improved decider to take direction of the car into POV
    """
    def direction_decider_v2(self, dx_thresh=1.3):
        dx, dy = self.motion_vector
        gr_oncoming = 1.001
        
        if dy > 0.5:
            if dx < -dx_thresh and self.width_box_growth > gr_oncoming:
                return "ONCOMING"
            
            if dx > dx_thresh:
                if abs(self.real_speed) < 5.0:
                    return "STATIONARY"
                else:
                    return "OVERTAKING"
    ...
    def update_metrics(self, raw_distance, delta_time, ego_speed_ms):

        current_centroid = self.calculate_centroid()
        raw_motion = (current_centroid[0] - self.prev_centroid[0], current_centroid[1] - self.prev_centroid[1])
        mot_x = self._low_pass_filter(raw_motion[0], self.motion_vector[0], self.alpha_motion)
        mot_y = self._low_pass_filter(raw_motion[1], self.motion_vector[1], self.alpha_motion)
        self.motion_vector = (mot_x, mot_y)
        ...
        if delta_time > 0:
            ...
            raw_real_speed = ego_speed_ms + self.speed
        ...
        self.width_box_growth = self._low_pass_filter(raw_growth, self.width_box_growth, 0.1)
        self.prev_box_width = actual_width

        self.status = self.direction_decider_v2()
